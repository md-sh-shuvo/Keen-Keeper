                ...
                friendsData.map((friend, idx) =>
                    <div key={idx} className="">
                        <div className="bg-white rounded-lg shadow-lg p-8">
                            ...
                            <p className="flex gap-2 justify-center items-center my-[6px]">{friend.tags.map((tag, idx) => <span key={idx} className="bg-[#CBFADB] text-[#244D3F] text-[12px] font-medium py-1 px-3 rounded-full">{tag}</span>)}</p>
                            <span className="bg-[#EF4444] py-1 px-3 text-white rounded-full text-[12px] font-medium">{friend.status}</span>
                        </div>
                    </div>
                )
            }
